...
class TreeSearchEnv:
    ...
    def _branch(self):
        ...
        parent.child_idx = (parent.child_idx + 1) % len(parent.children)
    ...
    def _prepare_constraints(self, nodes: list[Node]):
        ...
        for node in nodes:
            ...
            if node.state.is_terminal():
                mask[0] = 0
            # terminate is valid only when the node is a terminal node or node reach depth limit
            mask[-1] = 1 if node.state.is_terminal() or node.depth == self.max_depth else 0
